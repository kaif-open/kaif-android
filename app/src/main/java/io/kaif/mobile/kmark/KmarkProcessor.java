...
public class KmarkProcessor {
  ...
  public static CharSequence process(Context context, final String input) {
    try {
      return new KmarkProcessor().process(context, new StringReader(input));
  ...
  private CharSequence process(Context context, final Reader reader) throws IOException {
    Configuration configuration = new Configuration.Builder().setDecorator(//
        new DefaultDecorator((int) Views.convertDpToPixel(10, context),
            (int) Views.convertDpToPixel(4, context),
            (int) Views.convertDpToPixel(2, context),
            context.getResources().getColor(R.color.code_gray))).build();
  ...
  private void initListBlock(final Block root) {
    BlockType blockType = root.type == BlockType.ORDERED_LIST
    ...
    while (line != null) {
      ...
      if ((t == LineType.OLIST || t == LineType.ULIST) || (!line.isEmpty && (line.prevEmpty
          && line.leading == 0))) {
        root.split(line.previous).type = blockType;

      }
      line = line.next;
    }
    root.split(root.lineTail).type = blockType;
  }
